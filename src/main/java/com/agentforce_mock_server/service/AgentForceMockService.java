...
public class AgentForceMockService {
  ...
  public Mono<ResponseDTO<TechnologyDTO>> getTechnologyFilters() {
    return agentForceMockRepository
        .getTechnologyFilters()
        .map(technologyDTOS -> ResponseDTO.<TechnologyDTO>builder().result(technologyDTOS).build());
  }

  public Mono<ResponseDTO<GeographyDTO>> getGeographyFilters() {
    return agentForceMockRepository
        .getGeographyFilters()
        .map(geographyDTOS -> ResponseDTO.<GeographyDTO>builder().result(geographyDTOS).build());
  }

  public Mono<ResponseDTO<IndustryDTO>> getIndustryFilters() {
    return agentForceMockRepository
        .getIndustryFilters()
        .map(industryDTOS -> ResponseDTO.<IndustryDTO>builder().result(industryDTOS).build());
  }

  public Mono<ResponseDTO<AccountsDTO>> getAccounts(AccountsReq accountsReq) {
    return agentForceMockRepository
        .getAccounts(accountsReq)
        .map(accountsDTOS -> ResponseDTO.<AccountsDTO>builder().result(accountsDTOS).build());
  }
}
